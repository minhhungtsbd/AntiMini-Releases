 ...
 * to antimini-sync; client `list()` calls strip it from results so it never
 * leaks into application data.
 */
const MANIFEST_KEY = ".antimini-sync-manifest";
...
export class SyncService implements OnModuleInit {
  ...
  constructor(private configService: ConfigService) {
    ...
    this.bucket =
      this.configService.get<string>("S3_BUCKET") || "antimini-sync";
  ...
  async list(dto: ListRequestDto, ctx?: UserContext): Promise<ListResponseDto> {
    ...
    const objects = (response.Contents || [])
      // Don't leak antimini-sync's internal manifest object to clients.
